...
image = mpimg.imread('quadrate_250_250_100pps_profiles.png')
...
featurex, featurey = np.genfromtxt('features1_quadrate_profile.txt', unpack = True)
featurex *= 1e6
...
featurex, featurey = np.genfromtxt('features2_quadrate_profile.txt', unpack = True)
featurex *= 1e6
...
upper_mask = (x2 < featurex[0] - 0.4) | (x2 > featurex[-1] + 0.4)
lower_mask = x2 == y2
for i in [1]:
