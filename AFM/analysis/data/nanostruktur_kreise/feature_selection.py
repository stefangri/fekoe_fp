...
fig, ax = plt.subplots()
x1, y1, x2, y2 = np.genfromtxt(sys.argv[1], unpack = True)
ax.plot(x1, y1)
featurex, featurey = select_features(fig, ax)
plt.show()
np.savetxt('features1_' + sys.argv[1] + '.txt', np.column_stack([featurex, featurey]))
...
featurex, featurey = select_features(fig, ax)
plt.show()
np.savetxt('features2_' + sys.argv[1] + '.txt', np.column_stack([featurex, featurey]))
